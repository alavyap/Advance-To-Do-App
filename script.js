document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadFocusedMode() {
    focusedModeActive =
      localStorage.getItem(FOCUSED_MODE_STORAGE_KEY) === "true";
  }
  ...
  function getVisibleOnHoldTasks() {
    ...
    return focusedModeActive
      ? onHold.filter((task) => task.focusedWork)
      : onHold;
  }
  ...
  function handlePointerDragEnd(event) {
    ...
    const droppedOnTaskId =
      dragTargetTaskId ||
      Number(
        getTaskItemFromPoint(event.clientX, event.clientY)?.dataset.taskId,
      );
  ...
  function renderTasks() {
    ...
    document.getElementById("completedSummaryCount").textContent =
      completedCount;
  ...
  document
    .querySelector(".focused-work-toggle")
